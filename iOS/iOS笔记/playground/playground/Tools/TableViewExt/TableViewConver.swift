...
public struct TableViewConvertTable: ArrayConvertAble, TableResponseAble {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if cell == nil, let nib = data.nibResource {
            tableView.register(nib, forCellReuseIdentifier: data.cellType)
            cell = tableView.dequeueReusableCell(withIdentifier: data.cellType)
        }
...
public struct TableViewConvertCell: DictionaryConvertAble {
    ...
    public private(set) var nibResource: UINib?
    ...
    init(cellType: String, cellClazz: AnyClass?, height: CGFloat, cellData: Any?) {
        ...
        if let c = cellClazz {
            let cs: String = NSStringFromClass(c)
            let r = cs.components(separatedBy: ".")
            if let l = r.last {
                if let _ = Bundle.main.path(forResource: l, ofType: "nib") {
                    self.nibResource = UINib.init(nibName: l, bundle: nil)
                }
            }
        }
    }
